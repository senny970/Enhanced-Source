...
PRECACHE_REGISTER_BEGIN( GLOBAL, PrecacheFlatBasicPortalDrawingMaterials )
...
{
	return GetPortalRender().ShouldUseStencilsToRenderPortals() &&
		m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration && 
		(GetPortalRender().GetRemainingPortalViewDepth() == 0) && 
		(GetPortalRender().GetViewRecursionLevel() > 1) &&
		(GetPortalRender().GetCurrentViewExitPortal() != this);
}
...
bool CPortalRenderable_FlatBasic::CalcFrustumThroughPortal( const Vector &ptCurrentViewOrigin, Frustum OutputFrustum )
{
	...
	int iViewRecursionLevel = GetPortalRender().GetViewRecursionLevel();
	...
	//VPlane *pInputFrustum = view->GetFrustum(); //GetPortalRender().m_RecursiveViewComplexFrustums[iViewRecursionLevel].Base();
	//int iInputFrustumPlaneCount = 6; //GetPortalRender().m_RecursiveViewComplexFrustums[iViewRecursionLevel].Count();
	VPlane *pInputFrustum = GetPortalRender().m_RecursiveViewComplexFrustums[iViewRecursionLevel].Base();
	int iInputFrustumPlaneCount = GetPortalRender().m_RecursiveViewComplexFrustums[iViewRecursionLevel].Count();
	...
	GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].SetCount( iVertCount + 2 ); //+2 for near and far z planes
	...
	for( i = 0; i != iVertCount; ++i )
	{
		...
		GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init( vNormal, vNormal.Dot( ptTransformedCamera ) );
	}

	//Near Z
	GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init( m_pLinkedPortal->m_vForward, m_pLinkedPortal->m_InternallyMaintainedData.m_fPlaneDist );
	...
	++i;
	{
		...
		GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i).Init( vNormal, vNormal.Dot( m_matrixThisToLinked * ptOnPlane ) );
	}
	...
	if( iVertCount > 4 )
	{
		...
		memcpy( Planes, GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Base(), sizeof( VPlane ) * iVertCount );
	...
	else
	{
		for( i = 0; i != iVertCount; ++i )
		{
			OutputFrustum[i] = GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(i);
		}
	...
	int iComplexCount = GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Count();
	OutputFrustum[FRUSTUM_NEARZ] = GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(iComplexCount-2);
	OutputFrustum[FRUSTUM_FARZ] = GetPortalRender().m_RecursiveViewComplexFrustums[iNextViewRecursionLevel].Element(iComplexCount-1);
...
void CPortalRenderable_FlatBasic::RenderPortalViewToBackBuffer( CViewRender *pViewRender, const CViewSetup &cameraView )
{
...

	{
		...
		render->Push3DView( portalView, 0, NULL, pViewRender->GetFrustum() );		
		{
			...
			if( m_InternallyMaintainedData.m_bUsableDepthDoublerConfiguration && (GetPortalRender().GetRemainingPortalViewDepth() == 1) )
			{
...
void CPortalRenderable_FlatBasic::RenderPortalViewToTexture( CViewRender *pViewRender, const CViewSetup &cameraView )
{
	...
	float fPixelVisibilty = GetPortalRender().GetPixelVisilityForPortalSurface( this );
...

	{
...

		{
			...
			ViewDrawScene( pViewRender, bDrew3dSkybox, nSkyboxVisible, portalView, nClearFlags, (view_id_t)GetPortalRender().GetCurrentViewId(), false, 0, &customVisibility );
...
void CPortalRenderable_FlatBasic::DrawPreStencilMask( void )
{
	...
	{
		DrawSimplePortalMesh( m_Materials.m_Portal_Refract );
	}
}
...
void CPortalRenderable_FlatBasic::DrawStencilMask( void )
{
	...
	DrawRenderFixMesh( GetPortalRender().m_MaterialsAccess.m_WriteZ_Model );
}
...
void CPortalRenderable_FlatBasic::DrawPostStencilFixes( void )
{
	...
	//replace depth
	DrawSimplePortalMesh( GetPortalRender().m_MaterialsAccess.m_WriteZ_Model, 0.0f );
	DrawRenderFixMesh( GetPortalRender().m_MaterialsAccess.m_WriteZ_Model, 0.0f );
}
...
void CPortalRenderable_FlatBasic::DrawSimplePortalMesh( const IMaterial *pMaterialOverride, float fForwardOffsetModifier )
{
	...
	{
		pRenderContext->Bind( (IMaterial *)(const IMaterial *)GetPortalRender().m_MaterialsAccess.m_Wireframe, (CPortalRenderable*)this );
...
void CPortalRenderable_FlatBasic::RenderFogQuad( void )
{
	...
	pRenderContext->Bind( (IMaterial *)(const IMaterial *)GetPortalRender().m_MaterialsAccess.m_TranslucentVertexColor );
...
void CPortalRenderable_FlatBasic::DrawPortal( void )
{
	...
	if ( GetPortalRender().ShouldUseStencilsToRenderPortals() )
	{
		...
		if( GetPortalRender().IsRenderingPortal() == false ) //main view
		{
		...
		else if( GetPortalRender().GetCurrentViewExitPortal() != this )
		{
			if( m_pLinkedPortal == NULL ) //didn't pass through pre-stencil mask
			{
				if ( ( m_fOpenAmount > 0.0f ) && ( m_fOpenAmount < 1.0f ) )
	...
	else
	{
		...
		if( GetPortalRender().IsRenderingPortal() == false )
		{			
			if ( ( m_fOpenAmount > 0.0f ) && ( m_fOpenAmount < 1.0f ) )
			{
				DrawSimplePortalMesh( m_Materials.m_Portal_Refract );
		...
		else if( GetPortalRender().GetCurrentViewExitPortal() != this ) //don't render portals that our view is exiting from
		{
			if ( ( m_fOpenAmount > 0.0f ) && ( m_fOpenAmount < 1.0f ) )
			{
				DrawSimplePortalMesh( m_Materials.m_Portal_Refract );
